...
async def games_page(request: Request, tg_id: int = None, name: str = None, db: Session = Depends(get_db)):
    # 1. Ищем игры, которые ждут игроков (как и было)
    waiting_games = db.query(GameSession).filter(GameSession.status == GameStatus.WAITING).all()
    
    # 2. Ищем активную игру именно для этого игрока
    active_game = None
    if tg_id:
        # Ищем игрока в базе
        player = db.query(Player).filter(Player.tg_user_id == tg_id).first()
        # Если игрок есть и его игра в процессе — запоминаем её
        if player and player.game_id:
            game = db.query(GameSession).filter(GameSession.id == player.game_id).first()
    ...
    return templates.TemplateResponse(
        ...
        context={
            "games": waiting_games,
            "active_game": active_game, # Передаем активную игру, если она есть
            "current_tg_id": tg_id,
            "current_name": name
        }
    )
...
async def lobby_players_partial(request: Request, game_id: int, tg_id: int = 123, name: str = "Выживший", db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse(
        ...
        context={"players": players, "game": game, "current_tg_id": tg_id, "x":y, "current_name": name}
    )
...
async def player_exit(
    ...
    """Удаляет игрока и перенаправляет только если это 'выход', а не 'кик'"""
    leave_player_from_game(db, tg_id)
    
    if redirect:
        # Используем f-строку, чтобы вставить реальные данные
        response.headers["HX-Redirect"] = f"/?tg_id={tg_id}&name={name}"
    
    return HTMLResponse("Выходим...")
@app.get("/admin/initgame")
async def new():
    seed_inventory()
    seed_extra_cards()
    finish = seed_biology_and_appearance(count=15)
    return finish 
...
async def use_card(
    ...
    if ability_card.interaction_type in rp_actions:
        narrative = f"Игрок {actor_player.name} применил '{ability_card.name}' на {target_player.name}! Результат: {outcome} (Кубик: {roll})."
    else:
        narrative = f"Игрок {actor_player.name} успешно использовал '{ability_card.name}' против {target_player.name}."

    # 3. ЗАПИСЫВАЕМ В БАЗУ ДЛЯ ВСЕХ
    game.last_action_log = narrative
...
async def admin_reset(response: Response, db: Session = Depends(get_db)):
    """Секретный эндпоинт для полного сброса игры и редиректа"""
    # Если ты хочешь именно УДАЛЯТЬ всё, используй hard_wipe_database, 
    # которую мы обсуждали ранее. Если просто сбросить — оставляй как есть.
    success = reset_entire_database(db) 
    
    if success:
        # Этот заголовок заставит HTMX перенаправить браузер на /admin
        response.headers["HX-Redirect"] = "/admin"
        return "Успешно"
    else:
        return HTMLResponse(content="<span style='color:red;'>Ошибка</span>", status_code=500)
    

@app.get("/admin/reset_all_admin")
async def admin_reset2(db: Session = Depends(get_db)):
    """Секретный эндпоинт для полного сброса игры"""
    success = reset_entire_database(db)
    
    if success:
        # Возвращаем простой скрипт, который выведет уведомление и вернет на главную
        return "База успешно обнулена: карты свободны, игры в ожидании!"
    else:
        return "Ошибка при обнулении базы"
...
async def admin_dashboard(request: Request, db: Session = Depends(get_db)):
    ...
    player = db.query(Player).filter(Player.tg_user_id == 123).first()
    # Если игрок есть и его игра в процессе — запоминаем её
    if player and player.game_id:
        game = db.query(GameSession).filter(GameSession.id == player.game_id).first()
        if game and game.status == GameStatus.IN_PROGRESS:
    ...
    return templates.TemplateResponse(
        ...
        context={
            ...
            "last_game": last_game # Передаем активную игру, если она есть
